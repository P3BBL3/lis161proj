...
def tierlist():
    ...
    return render_template('stlo.html', tier0Main=tier0main, tier0Vanguard=tier0vanguard, username=username, nav="ON", power=power)
...
def tierlistboss():
    ...
    tier0vanguard = getDBdata(openDB(2), 'SELECT * FROM tier0Vanguard')
    # This renders the stlo template while passing values received from database, the session values and the nav value.
    return render_template('stlb.html', tier0Main=tier0main, tier0Vanguard=tier0vanguard, username=username, nav="ON", power=power)
...
def tierlistmob():
    ...
    tier0vanguard = getDBdata(openDB(2), 'SELECT * FROM tier0Vanguard')
    # This renders the stlo template while passing values received from database, the session values and the nav value.
    return render_template('stlm.html', tier0Main=tier0main, tier0Vanguard=tier0vanguard, username=username, nav="ON", power=power)
...
def pvpcomp():
    username = session["username"]
    power = session["power"]
    return render_template('pvpcomp.html', username=username, nav="ON", power=power)
...
def eqloadout():
    username = session["username"]
    power = session["power"]
    return render_template('etl.html', username=username, nav="ON", power=power)
...
def eqtierlist():
    username = session["username"]
    power = session["power"]
    return render_template('geneq.html', username=username, nav="ON", power=power)
...
def about():
    ...
    if request.method == "GET":
        data = getDBdata(openDB(1), "SELECT * FROM comments")
        return render_template('about.html', username=username, nav="ON", power=power, data=data, check=0)
    # POST is for when a user goes to the page through submitting some kind of form.
        ...
        try:  # We are trying to see if the POST request we got is from inputting a comment.
            ...
            var_username = username  # While this is redundant, I just want TRY to be consistent with my naming.

            if var_comment == "":  # If there are no inputs, it will just reload page.
                data = getDBdata(openDB(1), "SELECT * FROM comments")
                return render_template('about.html', username=username, nav="ON", power=power, data=data, check=4)
            else:  # If there are changes, it will edit the comment.
                saveDBdata(openDB(1), "INSERT INTO comments (username,comment,link) VALUES ('" + var_username + "','" + var_comment + "'," + "'/about'" + ")")
                data = getDBdata(openDB(1), "SELECT * FROM comments")
                return render_template('about.html', username=username, nav="ON", power=power, data=data, check=1)
        # Deleting Comment =============================================================================================
        try:  # We are trying to see if the POST request we got is from pressing the delete button
            ...
            data = getDBdata(openDB(1), "SELECT * FROM comments")  # Loading the table without the deleted data
            return render_template('about.html', username=username, nav="ON", power=power, data=data, check=3)
        # Editing Comment ==============================================================================================
        try:  # We are trying to see if the POST request we got is from pressing the Edit button
            ...
            if var_edit == "":
                ...
                data = getDBdata(openDB(1), "SELECT * FROM comments")
                return render_template('about.html', username=username, nav="ON", power=power, data=data, check=2)
...
def signup():
    usercheck = 0  # Initialize the value of usercheck which will later be used for deciding what response the program will give.
    if request.method == "GET":  # When user enters the signup route through links.
        ...
        if usercheck == 1 or usercheck == 2 or usercheck == 3:  # If any of the three error is catched, the program will send an error message depending on which error.
            return render_template("signup.html", check=usercheck, nav="OFF")
...
def search():
    ...
    for datum in data:
        temp_tup = (datum[7],datum[2],"comment",datum[1],datum[4],datum[6])  # This arranges the data inside the tuples in a way that is convinient for me when putting them in the html.
        searchoutput.append(temp_tup)  # This puts the tuples into the list
        count += 1  # This counts how many search output there are.
